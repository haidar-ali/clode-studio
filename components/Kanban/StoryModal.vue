    ...
    <div v-if="isOpen" class="modal-overlay" @click="closeModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3>{{ editing ? 'Edit Story' : 'Create New Story' }}</h3>
          <button @click="closeModal" class="close-button">
            <Icon name="mdi:close" />
          </button>
        </div>
        
        <form @submit.prevent="saveStory" class="modal-form">
          <!-- Epic Assignment -->
          <div class="form-group" v-if="availableEpics.length > 0">
            <label for="story-epic">Epic</label>
            <select id="story-epic" v-model="form.epicId" class="form-select">
          ...
          <!-- User Story -->
          <div class="form-group">
            <label for="story-user-story">User Story *</label>
            <textarea
              id="story-user-story"
              v-model="form.userStory"
              rows="2"
              required
              placeholder="As a [user type], I want [functionality], so that [benefit]"
              class="form-textarea"
            ></textarea>
            <small class="field-hint">Follow the format: "As a... I want... so that..."</small>
          </div>
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <select id="story-priority" v-model="form.priority" class="form-select">
            ...
            <div class="form-group">
              ...
              <select id="story-status" v-model="form.status" class="form-select">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <select id="story-points" v-model.number="form.storyPoints" class="form-select">
                <option :value="undefined">Not Estimated</option>
            ...
            <div class="form-group">
              <label for="story-estimate">Time Estimate</label>
              <input
                id="story-estimate"
          ...
          <div class="form-group">
            ...
            <div class="criteria-editor">
              <textarea
                ...
                class="form-textarea"
              ></textarea>
              <small class="field-hint">Enter each criterion on a new line. Be specific and testable.</small>
            </div>
          </div>
          
          <!-- Dependencies -->
          <div class="form-group">
            <label>Story Dependencies</label>
          ...
          <!-- Tasks Preview -->
          <div class="form-group" v-if="editing && relatedTasks.length > 0">
            <label>Related Tasks ({{ relatedTasks.length }})</label>
          ...
          <div class="form-actions">
            ...
            <button 
              type="submit" 
              class="save-button"
              :disabled="!form.title || !form.userStory || !hasAcceptanceCriteria"
            >
...
const editing = computed(() => props.story !== null);
...
const hasAcceptanceCriteria = computed(() => {
...
// Watch for parent epic changes
watch(() => props.parentEpic, (epic) => {
  if (epic && !editing.value) {
    form.value.epicId = epic.id;
  }
}, { immediate: true });
...
const saveStory = () => {
  if (!form.value.title || !form.value.userStory || !hasAcceptanceCriteria.value) return;
