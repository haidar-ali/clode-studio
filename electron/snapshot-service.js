...
export class SnapshotService {
    ...
    setupIpcHandlers() {
        // Save snapshot
        ipcMain.handle('snapshots:save', async (_, snapshot) => {
            try {
        ...
        // List snapshots
        ipcMain.handle('snapshots:list', async (_, options) => {
            return this.handleListSnapshots(_, options);
        });
        // Delete snapshot
        ipcMain.handle('snapshots:delete', async (_, snapshotId, branch) => {
            try {
        ...
        ipcMain.handle('snapshots:update', async (_, snapshot) => {
            try {
                const branch = snapshot.gitBranch || 'main';
                const filename = `${snapshot.id}.json`;
                const filepath = path.join(this.getSnapshotsDir(branch), filename);
        ...
        // Export snapshots
        ipcMain.handle('snapshots:export', async (_, exportPath) => {
            try {
        ...
        // Import snapshots
        ipcMain.handle('snapshots:import', async (_, importPath) => {
            try {
        ...
        // Content compression
        ipcMain.handle('snapshots:compressContent', async (_, content) => {
            try {
        ...
        // Content storage
        ipcMain.handle('snapshots:storeContent', async (_, params) => {
            try {
        ...
        // Content retrieval
        ipcMain.handle('snapshots:getContent', async (_, params) => {
            try {
        ...
        // Diff storage
        ipcMain.handle('snapshots:storeDiff', async (_, params) => {
            try {
        ...
        // Diff retrieval
        ipcMain.handle('snapshots:getDiff', async (_, params) => {
            try {
        ...
        ipcMain.handle('snapshots:scanProjectFiles', async (_, params) => {
            try {
        ...
        ipcMain.handle('snapshots:setCurrentBranch', async (_, branch) => {
            try {
        ...
        // File restoration
        ipcMain.handle('snapshots:restoreFiles', async (_, params) => {
            try {
        ...
        ipcMain.handle('snapshots:getStorageInfo', async (_, params) => {
            try {
        ...
        // Cleanup
        ipcMain.handle('snapshots:cleanup', async (_, params) => {
            try {
    ...
    async calculateContentStats() {
        let totalSize = 0;
        let count = 0;
        async function walkContentDir(dir) {
            try {
        ...
        try {
            ...
            if (await fs.pathExists(gitignorePath)) {
                ...
                for (const dir of claudeDirectories) {
                    ...
                    const hasDir = lines.some(line => line === dirName ||
                        line === dir ||
                        line === '/' + dirName ||
                        line === '/' + dir);
                    if (!hasDir) {
     ...
     */
    matchesGitignorePattern(relativePath, patterns) {
    ...
    async handleListSnapshots(_, options) {
        try {
            const snapshots = [];
     ...
     */
    async detectFileInfo(filePath, buffer) {
        try {
     ...
     */
    fallbackDetection(fileName, ext) {
    ...
    // Content storage
    async storeContentObject(hash, content, mimeType, encoding, branch) {
    ...
    async getContentObject(hash, branch) {
        const contentPath = this.getContentPath(hash, branch);
        if (!await fs.pathExists(contentPath)) {
            return null;
        }
    ...
    // Diff storage
    async storeDiffObject(hash, diffObject, branch) {
    ...
    async getDiffObject(hash, branch) {
        const diffPath = this.getDiffPath(hash, branch);
    ...
    // File restoration
    async restoreProjectFiles(fileChanges, projectPath) {
        try {
