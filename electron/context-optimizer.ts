...
export interface TokenEstimate {
  content: string;
  tokens: number;
  type: 'message' | 'context' | 'file' | 'system';
}

export interface OptimizationStrategy {
  type: 'summarize' | 'truncate' | 'remove' | 'prioritize';
...
export class ContextOptimizer {
  private readonly CLAUDE_MAX_TOKENS = 200000;
  private readonly WARNING_THRESHOLD = 0.7; // 70% of max
  private readonly CRITICAL_THRESHOLD = 0.85; // 85% of max
  ...
  analyzeContextUsage(messages: any[], currentContext: string): {
    totalTokens: number;
    ...
    let status: 'normal' | 'warning' | 'critical' | 'danger' = 'normal';
    if (percentage >= 0.95) status = 'danger';
    else if (percentage >= this.CRITICAL_THRESHOLD) status = 'critical';
    else if (percentage >= this.WARNING_THRESHOLD) status = 'warning';
...
FILES: ${projectInfo.totalFiles}`;
  ...
  getOptimizationRecommendations(usage: ReturnType<typeof this.analyzeContextUsage>) {
  ...
  shouldInjectContext(
    ...
    if (hasContextKeyword) confidence += 0.3;
    if (isComplexQuery) confidence += 0.2;
    if (queryLower.includes('?')) confidence += 0.1;
