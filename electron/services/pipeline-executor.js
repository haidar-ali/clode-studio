...
export class PipelineExecutor extends EventEmitter {
    ...
    constructor(claudeService, codexService, worktreeManager) {
        super();
     ...
     */
    async setupEventDirectory(context) {
     ...
     */
    async setupFileWatcher(context) {
        const chokidar = await import('chokidar');
        const { eventPath, pipeline } = context;
        const watcher = chokidar.watch(path.join(eventPath, 'agents', '*', 'trigger.json'), {
            persistent: true,
            ignoreInitial: true,
            awaitWriteFinish: {
     ...
     */
    async executeSequential(context, order) {
     ...
     */
    async executeSmart(context, order) {
     ...
     */
    groupByDependencyLevel(pipeline) {
        const levels = [];
     ...
     */
    async executeAgent(context, agent) {
        try {
            // Update agent status
     ...
     */
    async spawnInstance(agent, workingDirectory) {
     ...
     */
    async spawnClaudeInstance(agent, instanceId, workingDirectory) {
     ...
     */
    async spawnCodexInstance(agent, instanceId, workingDirectory) {
     ...
     */
    async writeTriggerFile(context, agent) {
     ...
     */
    async createPipelineWorktree(pipeline) {
     ...
     */
    async createAgentWorktree(context, agent) {
     ...
     * Handle agent output
     */
    handleAgentOutput(context, agent, data) {
        const { eventPath } = context;
        const agentPath = path.join(eventPath, 'agents', agent.id);
        // Append to logs
     ...
     * Handle agent exit
     */
    async handleAgentExit(context, agent, code) {
     ...
     */
    async writeOutputFiles(context, agent) {
        if (!agent.config.outputFiles)
            return;
     ...
     * Handle agent error
     */
    handleAgentError(context, agent, error) {
        agent.status = 'failed';
        agent.execution.endTime = new Date();
     ...
     */
    async waitForAgentCompletion(context, agent) {
     ...
     * Check if pipeline is complete
     */
    async checkPipelineCompletion(context) {
        const { pipeline } = context;
        // Check if all agents are complete
        const allComplete = Array.from(pipeline.agents.values()).every(agent => agent.status === 'completed' || agent.status === 'failed' || agent.status === 'cancelled');
        if (allComplete) {
            pipeline.status = 'completed';
            pipeline.completedAt = new Date();
            // Check if any failed
            const anyFailed = Array.from(pipeline.agents.values()).some(agent => agent.status === 'failed');
     ...
     */
    async handleAgentStatusUpdate(context, agentId) {
        const { eventPath } = context;
    ...
    async resume(pipelineId) {
        const context = this.executionContexts.get(pipelineId);
        if (!context || context.pipeline.status !== 'paused')
            return;
