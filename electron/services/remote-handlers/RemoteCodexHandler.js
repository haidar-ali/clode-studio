...
export class RemoteCodexHandler {
    ...
    async cleanupSocketInstances(socketId) {
    ...
    async setupDesktopForwarding(socket, instanceId) {
        if (!this.mainWindow)
            return;
    ...
    async handleSpawn(socket, request, callback) {
        const { instanceId, workingDirectory, instanceName } = request.payload || {};
        const info = { instanceId, workingDirectory, instanceName, socketId: socket.id };
    ...
    async handleSend(_socket, request, callback) {
    ...
    async handleStop(_socket, request, callback) {
    ...
    async handleCreateInstance(_socket, request, callback) {
      ...
      `);
            if (id) {
                ...
                try {
                    this.mainWindow?.webContents?.send?.('codex:instances:updated');
                }
                catch { }
            }
            else {
    ...
    async handleResize(_socket, request, callback) {
    ...
    async stopInstance(instanceId) {
        ...
        if (global.codexInstances?.[instanceId]) {
            delete global.codexInstances[instanceId];
        }
        if (global.codexBuffers?.has(instanceId)) {
            global.codexBuffers.delete(instanceId);
        }
        ...
        if (set) {
            ...
            if (set.size === 0)
                this.instancesBySocket.delete(info.socketId);
        }
    }
    async handleGetBuffer(_socket, request, callback) {
        const { instanceId } = request.payload || {};
    ...
    async handleConfigureTerminal(_socket, request, callback) {
        const { instanceId, cols, rows } = request.payload || {};
        // In headless mode, check if the instance exists and is connected
        if (!this.mainWindow) {
            try {
                // Check if codex instance exists
                const codexInstances = global.codexInstances;
