...
export class FloatingWindowManager {
    ...
    setupIpcHandlers() {
        ...
        ipcMain.handle('floating-window:create', async (event, config) => {
            return this.createFloatingWindow(config);
        });
        // Close floating window
        ipcMain.handle('floating-window:close', async (event, moduleId) => {
            return this.closeFloatingWindow(moduleId);
        });
        // Update floating window
        ipcMain.handle('floating-window:update', async (event, moduleId, data) => {
        ...
        ipcMain.handle('floating-window:toggle-always-on-top', async (event, moduleId) => {
            const window = this.floatingWindows.get(moduleId);
            if (window && !window.isDestroyed()) {
        ...
        ipcMain.handle('floating-window:focus', async (event, moduleId) => {
            const window = this.floatingWindows.get(moduleId);
            if (window && !window.isDestroyed()) {
                window.focus();
                return true;
    ...
    saveWindowState(moduleId, window) {
        if (window.isDestroyed())
            return;
        const state = {
        ...
        this.store.set(`window-state-${moduleId}`, state);
    }
    loadWindowState(moduleId) {
        return this.store.get(`window-state-${moduleId}`);
    }
    ...
    getFloatingWindow(moduleId) {
        return this.floatingWindows.get(moduleId);
    }
    isModuleFloating(moduleId) {
        const window = this.floatingWindows.get(moduleId);
