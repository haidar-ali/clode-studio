...
ipcMain.handle('claude:getPendingOutput', async (event, instanceId) => {
...
ipcMain.handle('codex:start', async (event, instanceId, workingDirectory, instanceName) => {
    ...
    try {
        ...
        if (!global.pendingCodexOutput)
            global.pendingCodexOutput = new Map();
        p.onData((data) => {
...
ipcMain.handle('codex:send', async (event, instanceId, data) => {
    const p = codexInstances.get(instanceId);
    if (!p)
        return { success: false, error: `Codex instance is not running. Start it first.` };
    try {
        p.write(data);
        return { success: true };
    }
...
ipcMain.handle('codex:getPendingOutput', async (event, instanceId) => {
    if (global.pendingCodexOutput?.has(instanceId)) {
...
ipcMain.handle('codex:stop', async (event, instanceId) => {
    const p = codexInstances.get(instanceId);
    if (p) {
        try {
            p.kill();
...
ipcMain.handle('codex:resize', async (event, instanceId, cols, rows) => {
    const p = codexInstances.get(instanceId);
    if (p) {
        try {
            p.resize(cols, rows);
...
ipcMain.handle('codex:getBuffer', async (event, instanceId) => {
    try {
...
ipcMain.handle('codex:configureTerminal', async (event, instanceId, cols, rows) => {
    const p = codexInstances.get(instanceId);
    if (p) {
        try {
            p.resize(cols, rows);
...
ipcMain.handle('agent:initialize', async (event, workspacePath) => {
    try {
...
ipcMain.handle('agent:executeTask', async (event, action, params) => {
    try {
        ...
        switch (action) {
            ...
            case 'createTask':
                ...
                if (claudeCodeService) {
                    ...
                    const todos = await claudeCodeService.createTodosForTask(taskDescription, options.workspacePath || process.cwd());
                    return {
                        success: true,
                        result: {
                            taskId: task.id,
...
ipcMain.handle('monitoring:executeTask', async (event, action, params) => {
    try {
...
ipcMain.handle('knowledge:executeTask', async (event, action, params) => {
    try {
...
ipcMain.handle('contextBudgeter:executeTask', async (event, action, data) => {
    try {
