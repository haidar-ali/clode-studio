...
export function setupGitTimelineHandlers() {
    ...
    // Get timeline data
    ipcMain.handle('git-timeline:getData', async (event, workspacePath, filter) => {
        try {
    ...
    // Get commit details
    ipcMain.handle('git-timeline:getCommitDetails', async (event, workspacePath, hash) => {
        try {
            const service = getTimelineService(workspacePath);
    ...
    // Clear cache
    ipcMain.handle('git-timeline:clearCache', async (event, workspacePath) => {
        try {
            const service = getTimelineService(workspacePath);
            service.clearCache();
            return { success: true };
    ...
    // Checkout branch
    ipcMain.handle('git-timeline:checkoutBranch', async (event, workspacePath, branchName) => {
        try {
            const service = getTimelineService(workspacePath);
    ...
    // Create new branch
    ipcMain.handle('git-timeline:createBranch', async (event, workspacePath, branchName, startPoint) => {
        try {
            const service = getTimelineService(workspacePath);
            const git = service.git;
            if (startPoint) {
 ...
 */
export function cleanupGitTimelineService(workspacePath) {
