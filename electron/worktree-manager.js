...
export class WorktreeManager {
    ...
    sessions = new Map();
    activeWorktreePath;
    constructor(mainRepoPath, setupHandlers = true) {
    ...
    setupIpcHandlers() {
        ...
        // Create new worktree
        ipcMain.handle('worktree:create', async (event, branchName, sessionName, sessionDescription, metadata) => {
    ...
    async createWorktree(branchName, sessionName, sessionDescription, metadata) {
        try {
    ...
    async switchToWorktree(worktreePath) {
        try {
    ...
    async compareWorktrees(path1, path2) {
        try {
    ...
    async lockWorktree(worktreePath, lock) {
        try {
            const lockFile = path.join(worktreePath, '.git', 'locked');
            if (lock) {
    ...
    async listSessions() {
        try {
            ...
            for (const session of sessions) {
                const currentWorktree = worktrees.worktrees?.find(w => w.path === session.worktree.path);
                if (currentWorktree) {
    ...
    async setupWorktreeConfiguration(worktreePath) {
        try {
    ...
    findSessionByPath(worktreePath) {
