...
export class KnowledgeCacheService extends EventEmitter {
   ...
   */
  async learnFromQuery(query: string, result: any, responseTime: number, success: boolean) {
   ...
   */
  async predictNextQueries(currentContext: any): Promise<string[]> {
   ...
   * Learn from result structure
   */
  private learnFromResult(query: string, result: any) {
    // Track result types
  ...
  private getResultType(result: any): string {
    ...
    if (result.documentation) return 'documentation';
   ...
   */
  private matchesContext(pattern: string, context: any): boolean {
   ...
   * Update metrics
   */
  private updateMetrics(type: 'hit' | 'miss', responseTime: number) {
  ...
  private async saveMetrics() {
    try {
      ...
      await fs.writeFile(
        path.join(this.cachePath, 'metrics.json'),
...
export function createKnowledgeCache(workspacePath: string): KnowledgeCacheService {
